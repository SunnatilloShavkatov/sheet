...
class _ScrollableScope extends InheritedWidget {
  ...
  @override
  bool updateShouldNotify(_ScrollableScope old) => position != old.position;
}
...
class SheetState extends State<SheetScrollable>
  ...
  bool _shouldUpdatePositionBasedOnPhysics(SheetScrollable oldWidget) {
    ...
    do {
      if (newPhysics?.runtimeType != oldPhysics?.runtimeType || _shouldSheetPhysicsUpdate(newPhysics, oldPhysics)) {
        return true;
      }
  ...
  bool _shouldUpdatePositionBasedOnInitialExtent(SheetScrollable oldWidget) =>
      widget.initialExtent != oldWidget.initialExtent;

  bool _shouldUpdatePosition(SheetScrollable oldWidget) =>
      _shouldUpdatePositionBasedOnPhysics(oldWidget) || _shouldUpdatePositionBasedOnInitialExtent(oldWidget);
  ...
  double _targetScrollOffsetForPointerScroll(double delta) =>
      math.min(math.max(position.pixels + delta, position.minScrollExtent), position.maxScrollExtent);
  ...
  void _handlePointerScroll(PointerEvent event) {
    assert(event is PointerScrollEvent, 'Expected PointerScrollEvent, got $event');
        ...
        child: RawGestureDetector(
          ...
          child: Semantics(
            ...
            child: MinInteractionZone(
              ...
              child: IgnorePointer(ignoring: false, child: widget.viewportBuilder(context, position)),
            ),
          ),
        ),
      ),
...
class _ScrollSemantics extends SingleChildRenderObjectWidget {
  ...
  @override
  _RenderScrollSemantics createRenderObject(BuildContext context) => _RenderScrollSemantics(
    position: position,
