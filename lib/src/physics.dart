...
class NeverDraggableSheetPhysics extends NeverScrollableScrollPhysics with SheetPhysics {
  ...
  NeverDraggableSheetPhysics applyTo(ScrollPhysics? ancestor) =>
      NeverDraggableSheetPhysics(parent: buildParent(ancestor));
}
...
class AlwaysDraggableSheetPhysics extends AlwaysScrollableScrollPhysics with SheetPhysics {
  ...
  AlwaysDraggableSheetPhysics applyTo(ScrollPhysics? ancestor) =>
      AlwaysDraggableSheetPhysics(parent: buildParent(ancestor));
}
...
class BouncingSheetPhysics extends ScrollPhysics with SheetPhysics {
  ...
  bool shouldReload(covariant ScrollPhysics old) =>
      old is BouncingSheetPhysics && old.overflowViewport != overflowViewport;
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    ...
    final double overscrollPastStart = math.max(position.minScrollExtent - position.pixels, 0);
    final double overscrollPastEnd = math.max(position.pixels - position.maxScrollExtent, 0);
...
class SnapSheetPhysics extends ScrollPhysics with SheetPhysics {
  ...
  @override
  bool shouldReload(covariant ScrollPhysics old) =>
      old is SnapSheetPhysics && (old.relative != relative || !listEquals(old.stops, stops));
...
class SheetBehavior extends ScrollBehavior {
  ...
  @override
  SheetPhysics getScrollPhysics(BuildContext context) => _clampingPhysics;
}
